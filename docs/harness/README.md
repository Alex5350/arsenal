...
defined once in `docs/agents/` and reach each harness the same way:

| Harness | Agents directory | Mechanism |
| --- | --- | --- |
| Claude Code | `.claude/agents/<name>.md` | committed per-file symlink |
...
| Codex CLI | `.codex/agents/<name>.toml` | generated by bootstrap (TOML, gitignored) |
...
2. **Skills**: add the tool's project skill directory to the mapping table
   ...
   the skills. If the tool also supports custom agents, add its agent
   directory to `wire_agent_doors` the same way.
